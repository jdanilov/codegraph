 ...
 * siblings. Siblings are ordered largest-first (the DaisyDisk convention), so a
 * project reads as "where is the mass" at a glance.
 *
...
const RING_RADII: Array<{ r0: number; r1: number }> = (() => {
  const out: Array<{ r0: number; r1: number }> = [{ r0: 0, r1: CENTRE_RADIUS }];
  let radius = CENTRE_RADIUS;
  for (let ring = 1; ring <= MAX_RINGS; ring++) {
    const thickness = ringThickness(ring);
    out.push({ r0: radius, r1: radius + thickness });
    radius += thickness + RING_GAP;
  }
  return out;
})();

/** Outer radius of a full-depth disk, in layout units. */
export const MAX_RADIUS = RING_RADII[MAX_RINGS]!.r1;
...
/**
 * Split `span` among `children` (pre-sorted largest first).
 *
 ...
 * on the sliver floor. If they don't, the largest fill every slot but one and
 * the rest become the `+N smaller` arc.
 *
...
export function allocateSlots(
  ...
  const keptCount = fits ? children.length : Math.max(0, capacity - 1);
  const kept = children.slice(0, keptCount);
  const tail = children.slice(keptCount);
...
export function computeSunburst(
  ...
  for (let ring = 1; ring <= maxRings && frontier.length > 0; ring++) {
    ...
    for (const item of frontier) {
      ...
      const children = childIds
        .map((id) => ({ id, size: sizes.get(id) ?? 1 }))
        .sort((a, b) => b.size - a.size || compareNames(model, a.id, b.id));
  ...
  return {
    ...
    maxRadius: rings > 0 ? RING_RADII[rings]!.r1 : CENTRE_RADIUS,
  };
}
 ...
 * arcs) rather than the whole disk.
 */
export function arcAt(layout: SunburstLayout, x: number, y: number): SunburstArc | null {
  ...
  for (let ring = 1; ring <= layout.rings; ring++) {
    ...
    const first = arcsInRing[0]!;
    if (radius < first.r0 || radius > first.r1) continue;
